...
static CGFloat const MENU_APPEAR_ANIMATION_DURATION=0.2f;
static CGFloat const VISIBLE_WIDTH_CENTER_VIEW=50.0f;
...
- (void)setPlayerViewHidden:(BOOL)hidden
{
    ...
    self.playerViewTop.constant = CGRectGetHeight(frame) - PLAYER_VIEW_HEIGHT;
    
    [self.playerView setHidden:hidden];
}

- (BOOL)isPlayerViewHidden{
    return self.playerView.hidden;
}

- (void)showPlayerView
{
    if (_isPlayerViewSmall) {
        self.isPlayerViewSmall = NO;
        AppDelegate *delegate = [NSObject appDelegate];
        [delegate setIsShowVideo:YES];
        
        self.playerViewTop.constant = 0;
        
        [UIView animateWithDuration:PLAYER_ANIMATION_DURATION
                         animations:^{
...
{
    if (!_isPlayerViewSmall) {
        self.isPlayerViewSmall = YES;
        AppDelegate *delegate=[NSObject appDelegate];
        [delegate setIsShowVideo:NO];
        
        CGRect frame=self.playerView.frame;
        //_lastOffset = [[UIApplication sharedApplication] statusBarFrame].size.height - 20.0;
        if (_isSmallPlayerViewHidden) {
            self.playerViewTop.constant = CGRectGetHeight(frame);
        } else {
            self.playerViewTop.constant = CGRectGetHeight(frame) - PLAYER_VIEW_HEIGHT;
        }
        [UIView animateWithDuration:PLAYER_ANIMATION_DURATION
...
-(void)hideSmallPlayerAndNavBar:(FeedViewController*)vc{
    self.isSmallPlayerViewHidden = YES;
    if (_isPlayerViewSmall && !_isSmallPlayerViewHideAnimating) {
        [self.view layoutIfNeeded];
        _isSmallPlayerViewHideAnimating = YES;
        CGRect frame=self.playerView.frame;
      
        self.playerViewTop.constant = CGRectGetHeight(frame);
        vc.headerTopConstraint.constant = -60.0;
        ...
        [UIView animateWithDuration:PLAYER_ANIMATION_DURATION
                         ...
                         completion:^(BOOL finished) {
                            _isSmallPlayerViewHideAnimating = NO;
                             if (_isSmallPlayerViewHidden == NO) {
                                 [self showSmallPlayerAndNavBar:vc];
                             }
                         }];
    }
}

-(void)showSmallPlayerAndNavBar:(FeedViewController*)vc{
    self.isSmallPlayerViewHidden = NO;
    if (_isPlayerViewSmall && !_isSmallPlayerViewHideAnimating) {
        [self.view layoutIfNeeded];
        _isSmallPlayerViewHideAnimating = YES;
        CGRect frame=self.playerView.frame;
        ...
        self.playerViewTop.constant = CGRectGetHeight(frame) - PLAYER_VIEW_HEIGHT;
        vc.headerTopConstraint.constant = 0.0;
        ...
        [UIView animateWithDuration:PLAYER_ANIMATION_DURATION
                         ...
                         completion:^(BOOL finished) {
                             ...
                             _isSmallPlayerViewHideAnimating = NO;
                             if (_isSmallPlayerViewHidden == YES) {
                                 [self hideSmallPlayerAndNavBar:vc];
                             }
                         }];
    }
}
...
- (void)viewDidLoad {
    ...
    //self.lastOffset = [[UIApplication sharedApplication] statusBarFrame].size.height - 20.f;
    self.playerViewTop.constant = CGRectGetHeight(frame) - PLAYER_VIEW_HEIGHT;
    self.isPlayerViewSmall = YES;
    
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(appDidChangeStatusBarHeight:)
...
//    [self addChildViewController:_centerViewController];
    [self.view insertSubview:[_centerViewController view] belowSubview:_playerView];
...
- (void)alignLeftMenuControllerWithCenterViewController {
    ...
    leftMenuFrame.size.width = CGRectGetWidth([[UIScreen mainScreen]bounds]);
    
    leftMenuFrame.origin.x =0.0f;
...
//        }
//    }
    for(UIView *view in [(UIView*)[self.centerViewController view]subviews])
    {
        [view setUserInteractionEnabled:self.menuState==MFSideMenuStateClosed];
    }
}
...
#pragma mark - Center vie wshow/hide methods

- (void)hideCenterViewController:(void (^)(void))completion;
{
    [UIView animateWithDuration:MENU_APPEAR_ANIMATION_DURATION
                     animations:^{
                         [self setCenterViewControllerOffset:CGRectGetWidth(self.view.frame)];
                     }
                     completion:^(BOOL finished){
...
- (void)showCenterViewController:(void (^)(void))completion;
{
    [UIView animateWithDuration:MENU_APPEAR_ANIMATION_DURATION
                     animations:^{
...
- (void)setIsFullMenu:(BOOL)isFullMenu
{
    if(isFullMenu)
    {
        self.leftMenuWidth=CGRectGetWidth([[UIScreen mainScreen]bounds]);
    }
    else
    {
        self.leftMenuWidth=CGRectGetWidth([[UIScreen mainScreen]bounds])-VISIBLE_WIDTH_CENTER_VIEW;
    }
}

#pragma mark - Notification Center

- (void)appDidChangeStatusBarHeight:(NSNotification *)notification
{
    CGRect lastOffsetRect = [[notification.userInfo objectForKey:@"UIApplicationStatusBarFrameUserInfoKey"] CGRectValue];
    if (lastOffsetRect.size.height == 20) {
        //self.lastOffset = 20.f;
    } else {
        //self.lastOffset = 0;
    }
    
    if (_isPlayerViewSmall) {
        [self setPlayerViewHidden:self.playerView.isHidden];
    }
}
