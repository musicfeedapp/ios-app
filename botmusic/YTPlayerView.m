...
NSString static *const kYTPlayerPageURL = @"https://musicfeed.rubyforce.co/youtube.html?youtubeid=";

@interface YTPlayerView () <NSURLConnectionDelegate, NSURLConnectionDataDelegate> {
    BOOL _isDone;
...
- (BOOL)loadWithVideoId:(NSString *)videoId playerVars:(NSDictionary *)playerVars {
    ...
    NSString *urlString = [NSString stringWithFormat:@"%@%@", kYTPlayerPageURL, videoId];
    NSURL *youtubeUrl = [NSURL URLWithString:urlString];
    _urlRequest = [NSURLRequest requestWithURL:youtubeUrl];
...
- (BOOL)loadWithPlaylistId:(NSString *)playlistId playerVars:(NSDictionary *)playerVars {
  ...
  [tempPlayerVars addEntriesFromDictionary:playerVars];  // No-op if playerVars is null
    ...
    NSString *urlString = [NSString stringWithFormat:@"%@%@", kYTPlayerPageURL, playlistId];
    NSURL *youtubeUrl = [NSURL URLWithString:urlString];
    _urlRequest = [NSURLRequest requestWithURL:youtubeUrl];
    [self.webView loadRequest:_urlRequest];
...
- (void)notifyDelegateOfYouTubeCallbackUrl: (NSURL *) url {
  ...
  if (query) {
    ...
    if ([action isEqualToString:@"onYouTubeIframeAPIReady"]) {
        [self.delegate playerViewDidBecomeReady:self];
    }
  else if ([action isEqual:kYTPlayerCallbackOnReady]) {
...
- (void)connection:(NSURLConnection *)connection didReceiveAuthenticationChallenge:(NSURLAuthenticationChallenge *)challenge {
    // WebView request got authentication challenge
...
- (void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response {
    // Connection is done, load WebView request again
    _isDone = YES;
    [self.webView loadRequest:_urlRequest];
