...
- (void)fadeOutNotification:(TSMessageView *)currentView animationFinishedBlock:(void (^)())animationFinished
{
    ...
    if (![MFMessageManager sharedInstance].statusBarShouldBeHidden) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)((kTSMessageAnimationDuration - 0.100) * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
...
+ (BOOL)dismissActiveNoInternetConnectionNotificationWithConnectedMessage
{
    if ([[TSMessage sharedMessage].messages count] == 0) return NO;
    ...
    dispatch_async(dispatch_get_main_queue(), ^
                   {
                       if ([[TSMessage sharedMessage].messages count] == 0) return;
                       TSMessageView *currentMessage = [[TSMessage sharedMessage].messages objectAtIndex:0];
                       if ([currentMessage.title isEqualToString:NSLocalizedString(@"No internet connection",nil)]) {
                           currentMessage.titleLabel.text = NSLocalizedString(@"Connected!",nil);
                           currentMessage.backgroundBlurView.blurTintColor = [UIColor colorWithRGBHex:0x00e6a8];
                           dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                               if (currentMessage.messageIsFullyDisplayed)
                               {
                                   [[TSMessage sharedMessage] fadeOutNotification:currentMessage animationFinishedBlock:nil];
                               }
                           });
                       } else {
                           NSMutableArray* array = [[TSMessage queuedMessages] mutableCopy];
                           TSMessageView* offlineMesage;
                           for (TSMessageView* message in array) {
                               if ([message.title isEqualToString:NSLocalizedString(@"No internet connection",nil)]) {
                                   offlineMesage = message;
                                   break;
                               }
                           }
                           if (offlineMesage) {
                               [array removeObject:offlineMesage];
                               [[TSMessage sharedMessage] setMessages:array];
                           }
                       }
                   });
    return YES;
